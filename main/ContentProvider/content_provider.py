...
from ..output_models import Article, Paragraph, Answer, QuestionAnswer
...
class ContentProvider:
    ...
    def _parse_article(self, data) -> Article:
        ...
        for p in data['paragraphs']:
            ...
            for qa in p['qas']:
                ...
                qas.append(QuestionAnswer(answers=answers, question=qa['question'], id=qa['id']))
    ...
    def get_content(self, mock: bool = False) -> Article:
        ...
        with open('squad_data/dev-v1.1.json') as f:
            dev_data = json.load(f)

        if mock:
            return self._parse_article(dev_data['data'][0])
        else:
            return self._parse_article(dev_data['data'][random.randint(0, len(dev_data['data']) - 1)])
